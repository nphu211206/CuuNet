...
// Dynamic import react-globe.gl (needs window)
...
const WORLD_POINTS = worldDisasters.slice(0, 15).map((d) => ({
    ...
    severity: d.severity,
    label: `${d.location.province} — ${d.title}`,
    size: d.severity === "critical" ? 0.35 : d.severity === "high" ? 0.25 : 0.18,
    color: d.severity === "critical" ? "#FF4757" : d.severity === "high" ? "#FF6B6B" : d.severity === "medium" ? "#FFB800" : "#00D68F",
    people: d.affectedPeople >= 1000000
        ? `${(d.affectedPeople / 1000000).toFixed(1)}M`
        : d.affectedPeople >= 1000
...
// Connection arcs between related disasters
const ARCS = [
...
// === TOOLTIP HTML ===
function createTooltipHTML(point: typeof ALL_POINTS[0]) {
    ...
    return `
        <div style="font-family:Inter,system-ui,sans-serif;padding:12px 16px;background:rgba(255,255,255,0.97);backdrop-filter:blur(12px);border:1px solid #E2E8F0;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,0.12);min-width:180px;pointer-events:none">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">
                <span style="font-size:16px">${icon}</span>
                <span style="font-size:12px;font-weight:700;color:#0F172A;line-height:1.3">${point.label}</span>
            </div>
            <div style="display:flex;align-items:center;gap:8px">
                <span style="font-size:10px;padding:2px 8px;border-radius:4px;font-weight:700;background:${point.color}15;color:${point.color}">${sevLabel}</span>
                <span style="font-size:10px;color:#64748B">${point.people} người</span>
            </div>
        </div>
    `;
}
